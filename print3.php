...
<head>
    ...
    <title>สรุปวัตถุดิบ (ผัก) - รพ.เทพา</title>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .footer-line {
            height: 3px;
            background: linear-gradient(90deg, #276749, #68d391, #276749);
            margin-bottom: 15px;
            border-radius: 2px;
        }
...
<body>
    ...
    <div class="print-page">
        <div class="report-header">
            <div class="header-content">
                ...
                <div class="header-text">
                    <h1>รายงานสรุปวัตถุดิบ (ผัก)</h1>
                    <p class="hospital-name">โรงพยาบาลเทพา จังหวัดสงขลา</p>
                    <span class="report-type"><i class="fas fa-leaf"></i> วัตถุดิบประเภทผัก</span>
                </div>
            </div>
        </div>
        ...
        <div class="table-section">
            ...
            <!-- ส่วนวัตถุดิบผัก -->
            <div class="section-title">
                <i class="fas fa-carrot"></i> 2. วัตถุดิบ (ผัก)
            </div>
            ...
            <table class="data-table">
                ...
                <tbody>
                    ...
                    // ห้องพิเศษ - เฉพาะผัก (staple_type_id = 1)
                    if (!empty($values2)) {
                        $sql3 = "SELECT m.menu_name, s.staple_name, s.staple_serve, s.staple_yield,
                                f.factor_value
                                FROM staple s
                                ...
                                WHERE m.menu_id IN ($values2) AND s.staple_type_id = 1 
                    ...
                    // ห้องธรรมดา - เฉพาะผัก
                    if (!empty($values1)) {
                        $sql = "SELECT m.menu_name, s.staple_name, s.staple_serve, s.staple_yield,
                                f.factor_value
                                FROM staple s
                                LEFT JOIN menu m ON m.menu_id = s.menu_id
                                LEFT JOIN menu_type t ON t.menu_type_id = m.menu_type_id
                                LEFT JOIN factor f ON f.staple_name = s.staple_name AND f.menu_type_id = t.menu_type_id
                                WHERE m.menu_id IN ($values1) AND s.staple_type_id = 1 
                                ORDER BY s.staple_name";
